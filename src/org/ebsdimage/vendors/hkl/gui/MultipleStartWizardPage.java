...
public class MultipleStartWizardPage extends WizardPage {
    ...
    private class Add extends PlugIn {
        ...
        public void xRun() {
            ...
            FileDialog.setMultipleSelection(true);
            FileDialog.setFileSelectionMode(FileNameField.FILES_ONLY);
            FileDialog.setFilter(FileDialog.addFilter("*.ctf"));
            ...
            for (File file : files) {
                // Verify CTF before adding it
                if (!CtfLoader.isCtf(file)) {
                    showErrorDialog("The ctf file (" + file + ") is not valid.");
                    return;
                }
    ...
    /** Map key for the ctf files. */
    public static final String KEY_CTF_FILES = "ctfFiles";
    ...
    public MultipleStartWizardPage() {
        ...
        String text =
                "<html>This importer converts the acquisition parameters "
                        + "and data from a list of ctf files into EBSD-Image's EBSD multimap "
        ...
        add(new JLabel("Select the CTF files"), "wrap");
        ...
        addButton.setToolTipText("Add CTF(s) to the list");
        addButton.setPlugIn(new Add());
        add(addButton, "split 3, align right");

        removeButton = new Button(REMOVE_ICON);
        removeButton.setToolTipText("Remove selected CTF from the list");
        removeButton.setPlugIn(new Remove());
        add(removeButton);

        clearButton = new Button(CLEAR_ICON);
        clearButton.setToolTipText("Clear all CTF(s) from the list");
    ...
    public boolean isCorrect(boolean buffer) {
        if (fileList.getModel().getSize() == 0) {
            showErrorDialog("Please specify at least one CTF file.");
            return false;
        }

        if (buffer) {
            put(KEY_CTF_FILES, getFiles());
