...
/**
 * Operation to calculate the total intensity of the Hough peaks.
 * 
 * @author Philippe T. Pinard
 */
public class Sum extends IdentificationResultsOps {
    ...
    /**
     * Creates a new <code>Sum</code> result operation where an unlimited number
     * of peaks will be used in the calculations.
     */
    public Sum() {
        this(DEFAULT_MAX);
    }
    ...
    /**
     * Creates a new <code>Sum</code> result operation.
     * 
     * @param max
     *            maximum number of peaks to consider in the calculations
    ...
    /**
     * Calculates the total intensity of the Hough peaks.
     * 
     * @param exp
    ...
    public OpResult[] calculate(Exp exp, HoughPeak[] peaks) {
        ...
        if (peaks.length == 0)
            ...
            value = Stats.sum(intensities);
        }

        String name = "Peaks Sum [max=" + max + "]";
