...
public class HoughPeaksXmlFileXmlLoader implements ObjectXmlLoader {
    ...
    /**
     * Loads a <code>HoughPeaksXmlFile</code> operation from an XML
     * <code>Element</code>.
     * 
     * @param element
     *            an XML <code>Element</code>
     * @return a <code>HoughPeaksXmlFile</code> operation
     * @throws IllegalNameException
     *             if the <code>Element</code> tag name is incorrect.
     */
    @Override
    public HoughPeaksXmlFile load(Element element) {
        if (!element.getName().equals(TAG_NAME))
            throw new IllegalNameException("Name of the element should be "
                    + TAG_NAME + " not " + element.getName() + ".");

        return new HoughPeaksXmlFile();
    }
