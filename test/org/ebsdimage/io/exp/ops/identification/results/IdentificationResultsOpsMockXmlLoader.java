...
public class IdentificationResultsOpsMockXmlLoader implements ObjectXmlLoader {

    @Override
    public IdentificationResultsOpsMock load(Element element) {
        if (!element.getName().equals(TAG_NAME))
            throw new IllegalNameException("Tag should be " + TAG_NAME
                    + ", not " + element.getName() + ".");

        return new IdentificationResultsOpsMock();
    }
