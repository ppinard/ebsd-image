...
public class PatternResultsOpsMockXmlLoader implements ObjectXmlLoader {

    @Override
    public PatternResultsOpsMock load(Element element) {
        if (!element.getName().equals(TAG_NAME))
            throw new IllegalNameException("Tag should be " + TAG_NAME
                    + ", not " + element.getName() + ".");

        return new PatternResultsOpsMock();
    }
