...
public abstract class ExpXmlTester extends ExpTester {
    ...
    public static Element createElement() {
        ...
        // Metadata
        element.addContent(new ExpMetadataXmlSaver().save(ExpTester
                .createMetadata()));
        ...
        parent.addContent(new PatternOpMockXmlSaver()
                .save(new PatternOpMock(0)).setText("1"));
        parent.addContent(new PatternOpMockXmlSaver()
                .save(new PatternOpMock(1)).setText("2"));
        element.addContent(parent);

        // Pattern Post Ops
        parent = new Element(PatternPostOpsXmlTags.TAG_NAME);
        parent.addContent(new PatternPostOpsMockXmlSaver()
                .save(new PatternPostOpsMock()));
        element.addContent(parent);

        // Pattern Results Ops
        parent = new Element(PatternResultsOpsXmlTags.TAG_NAME);
        parent.addContent(new PatternResultsOpsMockXmlSaver()
                .save(new PatternResultsOpsMock()));
        element.addContent(parent);

        // Hough Pre Ops
        parent = new Element(HoughPreOpsXmlTags.TAG_NAME);
        parent.addContent(new HoughPreOpsMockXmlSaver()
                .save(new HoughPreOpsMock()));
        element.addContent(parent);
        ...
        parent.addContent(new HoughPostOpsMockXmlSaver()
                .save(new HoughPostOpsMock()));
        element.addContent(parent);

        // Hough Results Ops
        parent = new Element(HoughResultsOpsXmlTags.TAG_NAME);
        parent.addContent(new HoughResultsOpsMockXmlSaver()
                .save(new HoughResultsOpsMock()));
        element.addContent(parent);

        // Detection Pre Ops
        parent = new Element(DetectionPreOpsXmlTags.TAG_NAME);
        parent.addContent(new DetectionPreOpsMockXmlSaver()
                .save(new DetectionPreOpsMock()));
        element.addContent(parent);

        // Detection Op
        parent = new Element(DetectionOpXmlTags.TAG_NAME);
        parent.addContent(new DetectionOpMockXmlSaver()
                .save(new DetectionOpMock()));
        element.addContent(parent);

        // Detection Post Ops
        parent = new Element(DetectionPostOpsXmlTags.TAG_NAME);
        parent.addContent(new DetectionPostOpsMockXmlSaver()
                .save(new DetectionPostOpsMock()));
        element.addContent(parent);

        // Detection Results Ops
        parent = new Element(DetectionResultsOpsXmlTags.TAG_NAME);
        parent.addContent(new DetectionResultsOpsMockXmlSaver()
                .save(new DetectionResultsOpsMock()));
        element.addContent(parent);

        // Identification Pre Ops
        parent = new Element(IdentificationPreOpsXmlTags.TAG_NAME);
        parent.addContent(new IdentificationPreOpsMockXmlSaver()
                .save(new IdentificationPreOpsMock()));
        element.addContent(parent);

        // Identification Op
        parent = new Element(IdentificationOpXmlTags.TAG_NAME);
        parent.addContent(new IdentificationOpMockXmlSaver()
                .save(new IdentificationOpMock()));
        element.addContent(parent);

        // Identification Post Ops
        parent = new Element(IdentificationPostOpsXmlTags.TAG_NAME);
        parent.addContent(new IdentificationPostOpsMockXmlSaver()
                .save(new IdentificationPostOpsMock()));
        element.addContent(parent);

        // Identification Results Ops
        parent = new Element(IdentificationResultsOpsXmlTags.TAG_NAME);
        parent.addContent(new IdentificationResultsOpsMockXmlSaver()
                .save(new IdentificationResultsOpsMock()));
        element.addContent(parent);

        // Indexing Pre Ops
        parent = new Element(IndexingPreOpsXmlTags.TAG_NAME);
        parent.addContent(new IndexingPreOpsMockXmlSaver()
                .save(new IndexingPreOpsMock()));
        element.addContent(parent);

        // Indexing Op
        parent = new Element(IndexingOpXmlTags.TAG_NAME);
        parent.addContent(new IndexingOpMockXmlSaver()
                .save(new IndexingOpMock()));
        element.addContent(parent);

        // Indexing Post Ops
        parent = new Element(IndexingPostOpsXmlTags.TAG_NAME);
        parent.addContent(new IndexingPostOpsMockXmlSaver()
                .save(new IndexingPostOpsMock()));
        element.addContent(parent);

        // Indexing Results Ops
        parent = new Element(IndexingResultsOpsXmlTags.TAG_NAME);
        parent.addContent(new IndexingResultsOpsMockXmlSaver()
                .save(new IndexingResultsOpsMock()));
        element.addContent(parent);
